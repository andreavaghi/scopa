...
Turns.bestMatch = function(matches) {
  ...
  for (var i = 0; i < matches.length; i++) {
    ...
    for (var j = 0; j < match.length; j++)
      if (match[j].suit === 'Quadri' && match[j].value === 7) return match;

    var coinCount = match.filter(function(card) {
      return card.suit === 'Quadri'
    }).length;

    if (coinCount > mostOri[0]) mostOri = [coinCount, match];
    if (match.length > mostCards[0]) mostCards = [match.length, match];
  }

  return (mostCards[0] > mostOri[0]) ? mostCards[1] : mostOri[1];
};
